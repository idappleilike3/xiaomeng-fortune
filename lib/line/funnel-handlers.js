...
function hasAdequateDescription(text) {
  return countDescribeChars(text) >= MIN_DESCRIBE_CHARS;
}

/**
 * Split long spoken copy into 2–3 LINE text bubbles (UX; stay under reply cap).
 * @param {string} text
 * @param {number} [maxBubbles]
 * @param {number} [softMax]
 */
function splitTextBubbles(text, maxBubbles = 3, softMax = 900) {
  ...
  return chunks.map((t) => textMessage(t.slice(0, 4800)));
}

/**
 * Text (spoken) first — may be 2–3 bubbles — then Flex with card face + next.
 * @param {object} card
 * @param {number} index
...
function cardRevealMessages(card, index, isLast, opts = {}) {
  ...
  const spoken = `${intro}\n\n${body}`;
  return [...splitTextBubbles(spoken), cardRevealFlex(card, index, isLast)];
}

/** Layer 3 — plain text only (no Flex). */
...
function describeTooShortMessages() {
  return [
    textMessage(
      "還想再聽你多說一點\n你剛講的有點短 我比較難對到真正卡住的地方\n\n可以再補：最近發生什麼、你現在什麼感覺、最想弄清楚的是哪一件\n大概再寫到四十個字以上就好 說清楚之後我們再抽牌"
    ),
  ];
}
...
export async function handleFunnelEvent(event, ctx) {
  ...
  if (event.type === "message" && event.message?.type === "text") {
    ...
    if (session.stage === "describe") {
      if (!hasAdequateDescription(text)) {
        // Soft nudge — no long delay (user is still mid-typing the story)
        return describeTooShortMessages();
      }
