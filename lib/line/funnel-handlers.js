...
const RESTART_KEYWORDS = /^(重新開始|重新解碼|再解碼一次)$/i;
...
/**
 * Free 3-card reveal already finished for this session.
 * Note: stage===need after「先不抽牌」is NOT completed — freeDrawDone stays false
 * so「還是想抽牌」can still run once.
 * @param {import('./funnel-session.js').FunnelSession} session
 */
function isFreeDrawCompleted(session) {
  return Boolean(session?.freeDrawDone);
}

/**
 * Resume need / recommend after free draw — never reshuffle.
 * @param {import('./funnel-session.js').FunnelSession} session
 * @param {ReturnType<typeof createFunnelContext>} ctx
 */
function resumeAfterFreeDrawMessages(session, ctx) {
  if (session.needId || session.stage === "recommend" || session.stage === "checkout") {
    const topicId = session.topicId || "love";
    const needId = session.needId || "solve_now";
    const products = pickRecommendations(needId, topicId);
    updateSession(session, {
      freeDrawDone: true,
      needId,
      productId: products[0]?.id || session.productId || null,
      stage: "recommend",
    });
    return [
      textMessage(
        "這輪免費三張已經抽過了喔\n我先帶你回到剛才的推薦；若要重來一輪 再點「重新開始」"
      ),
  ...
  updateSession(session, { freeDrawDone: true, stage: "need" });
  return [
    textMessage(
      "這輪免費三張已經抽過了喔\n接著選你現在最想要的幫忙；若要重來一輪 再點「重新開始」"
    ),
    needSelectTextMessage(),
    restartDecodeFlex(),
  ];
...
/**
 * Refuse a second free draw; point to need/recommend or explicit restart.
 * @param {import('./funnel-session.js').FunnelSession} session
 * @param {ReturnType<typeof createFunnelContext>} ctx
 */
function refuseRedrawMessages(session, ctx) {
  return [
    textMessage(
      "這輪免費三張已經抽完了 不會再幫你重抽一次喔\n要繼續的話選需求或看推薦；真的要重來再點「重新開始」"
    ),
    ...resumeAfterFreeDrawMessages(session, ctx).slice(1),
  ];
}

/**
 * Begin a fresh funnel from topic select (explicit restart / first start only).
 * @param {string} userId
 */
function beginFreshTopicFlow(userId) {
  resetSession(userId);
...
export async function handleFunnelEvent(event, ctx) {
  ...
  if (event.type === "message" && event.message?.type === "text") {
    ...
    if (RESTART_KEYWORDS.test(text)) {
      return handleFunnelPostback(userId, { funnel: "restart" }, ctx);
    }
...
async function handleFunnelPostback(userId, data, ctx) {
  ...
  switch (action) {
    ...
    case "start_decode": {
      ...
      // After free draw: rich menu /「開始解碼」must NOT silently restart draw
      if (isFreeDrawCompleted(session)) {
        return resumeAfterFreeDrawMessages(session, ctx);
      }
      // Mid-reveal: resume current card instead of wiping the draw
      if (session.stage === "cards" && Array.isArray(session.cards) && session.cards.length) {
        ...
        return [
          textMessage("這輪牌還在進行中 我們從剛才這張繼續"),
          ...cardRevealMessages(session.cards[i], i, i >= session.cards.length - 1),
        ];
      }
      return beginFreshTopicFlow(userId);
    }

    case "restart": {
      // Explicit new session only — used after freeDrawDone or「重新開始」
      return beginFreshTopicFlow(userId);
    }
    ...
    case "topic": {
      // Rich menu shortcuts (情感分析 / 萌寵) must not wipe a completed free draw
      if (isFreeDrawCompleted(session)) {
        return resumeAfterFreeDrawMessages(session, ctx);
      }
    ...
    case "free_yes": {
      if (isFreeDrawCompleted(session)) {
        return refuseRedrawMessages(session, ctx);
      }
      ...
      updateSession(session, { cards, cardIndex: 0, stage: "cards", freeDrawDone: false });
    ...
    case "after_cards": {
      updateSession(session, { stage: "need", freeDrawDone: true });
      return maybeHumanDelay(
        [
          textMessage(
    ...
    case "light_member": {
      updateSession(session, { stage: "welcome", freeDrawDone: true });
      return [
