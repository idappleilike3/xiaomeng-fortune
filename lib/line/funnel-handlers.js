 ...
 * TODO: Persist sessions + Relationship Case; wire real ECPay / points checkout.
 */
...
export function handleFunnelEvent(event, ctx) {
  ...
  // follow: let server.js compose legacy welcome + temple CTA (postback 開始解碼)
...
function handleFunnelPostback(userId, data, ctx) {
  ...
  switch (action) {
    case "start":
    case "start_decode": {
      // If pid present → unlock path; else start topic flow
      if (data.pid) {
        updateSession(session, { productId: data.pid, stage: "start" });
        return [
          startUnlockFlex({
            productId: data.pid,
            lineOaUrl: ctx.lineOaUrl,
            pricingUrl: ctx.pricingUrlFor(getProduct(data.pid)?.pricingRoute || "/pricing"),
          }),
        ];
      }
      resetSession(userId);
    ...
    case "light_member": {
      // Stub: mark light member locally; no payment API yet
      updateSession(session, { stage: "welcome" });
      return [
        textMessage("已為你建立 LINE 輕會員（本機紀錄）\n正式會員資料庫與付款 API 接上後會自動同步"),
