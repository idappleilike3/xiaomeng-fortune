...
import { updateSession } from "./funnel-session.js";
 ...
 * Passes conversation_id when known so the Chat App / KB stay continuous per LINE user.
 * @param {string} query
 * @param {string} userId
 * @param {string|null|undefined} conversationId
 * @returns {Promise<{ answer: string, conversationId: string|null }|null>}
 */
async function callDifyChat(query, userId, conversationId) {
  ...
  const json = await resp.json();
  const answer = String(json?.answer || "").trim() || null;
  if (!answer) return null;
  return {
    answer,
    conversationId: json?.conversation_id ? String(json.conversation_id) : conversationId || null,
  };
}
...
export async function generateFunnelChatText(input) {
  ...
  try {
    ...
    if (process.env.DIFY_API_KEY) {
      ...
      if (result?.answer) {
        if (result.conversationId && result.conversationId !== session.difyConversationId) {
          updateSession(session, { difyConversationId: result.conversationId });
        }
